...
#############################################################################
    ...
    #############################################################################
        ...
        self.inic = Person()
        self.persona = People()
        self.detection = PersonArray()
        # subscriptions
        rospy.Subscriber("/people_tracked", PersonArray, self.detectCallback)
    ...
    #############################################################################
        ...
        for j in range(0,len(self.detection.people)):
            if j <= (len(self.detection.people)-1):
                new_person = Person() 
                new_person.position.x = self.detection.people[j].pose.position.x 
                new_person.position.y = self.detection.people[j].pose.position.y
